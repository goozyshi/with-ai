...
> [不想看文字？没有关系，博客提供视频版了！点击直接进入](https://link.juejin.cn?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1K24y1q7eJ%2F%3Fvd_source%3D391a8dc379e0da60c77490e3221f097a)
...
首先是 《Vue.js 设计与实现》：它不同于市场上纯粹的 “源码分析” 类的图书。而是 **从高层的设计角度，探讨框架需要关注的问题（-尤雨溪序）** ，以 **提出问题 - 分析思路 - 解决问题** 的方式，来讲解 `vue 3` 的核心设计。其内部，没有读一行 `vue3` 的源码，但却可以让我们对整个 `vue 3` 的核心，拥有一个非常清楚的认知。
...
同时，最后请允许我打一个小广告，如果你不想看书，想要通过视频的方式学习 `vue 3` 核心逻辑，可以去看一下我在 慕课网的 [Vue3 源码解析，打造自己的 Vue3 框架](https://link.juejin.cn?target=https%3A%2F%2Fcoding.imooc.com%2Fclass%2F608.html) 这门课程。我个人 **强烈推荐** ！
...
- 第三篇：主要针对 `vue` 的渲染器（`renderer`），额外还包含了 `diff` 算法的详细讲解
...
![image-20230207231540415.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/21098e882769415382c0f4bdc2de531c~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

而 **声明式** 指的就是： **关注结果** 的范式。

![image-20230207231536123.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bfc0422aef1d4a6ba926c5fc0d064041~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
![image-20230207233547097.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f71e1cfd77484383b61e21ac9d285764~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
- 声明式的模板描述 ![image-20230208170232727.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bb128b7bae9c4b5393771c2e8971e749~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
- 命令式的 render 函数 ![image-20230208170236795.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1b2d6305c8f64e5589e522c8e29fe985~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
![image-20230208190429676.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/057596e640e945f8be97131e14a08043~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
> 首先是 `getter` 形式：
>
> ![image-20230208191120105.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f74a8b38bf694acfb2d617a2df6335a6~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
>
> 在该函数中，存在一个 `effect` 方法，方法内部触发了 `getter` 行为。一旦 `getter` 行为被触发，则把对应的 `effect` 方法保存到一个 “桶（数据对象）” 中
>
> 当触发 `setter` 行为时：
>
> ![image-20230208191257788.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6ebbfa33f9914324b6297d51d314037c~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
>
...
![image-20230208192343242.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f47258d57f1d4e6b88c2c039f14739e6~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
```scss
scss 体验AI代码助手 代码解读 复制代码if (!lazy) {
  // 执行副作用函数
}
```
...
![image-20230208194352049.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5680d49b18f24eeea80e24a2c98bfe80~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

> 这段代码完成的是一个异步操作。
>
> 如果 `obj` 连续被修改了两次，那么就会发起两个请求。我们最终的期望应该是 `data` 被赋值为 请求 B 的结果。
>
...
![image-20230209093545078.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0beec775a5bd428a90f42ca18ef8a0e4~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
这个代码比较多，大家如果想要查看具体代码的话，可以 [在 github 搜索 vue-next-mini](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Flgd8981289%2Fvue-next-mini)，进入到 `packages/runtime-dom/src/modules/events.ts` 路径下查看。
...
- 其次是 [收费的视频版](https://link.juejin.cn?target=https%3A%2F%2Fcoding.imooc.com%2Fclass%2F608.htm)，是在慕课网发布的。**同样链接会放到评论区**。
...
![image-20230209105953064.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3f6298754551403f94e277900fa988d1~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

而对于组件而言，同样需要使用 `vnode` 来进行表示，当 `vnode` 的 `type` 属性是一个 **自定义对象** 时，那么这个 `vnode` 就表示组件的 `vnode`

![image-20230209110548502.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/647d3c842b524c67b9fe6e7d1ceaefb1~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
![image-20230209110919710.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6dec186f3cd34fe69608d8c075fd3541~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
![image-20230209113241592.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/948936479d5d45d88d2795f47dabc39f~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

而对于 `vue` 的编译器而言，因为它是一个特定领域下的编译器，所以流程会进行一些优化，一共分为三大步

![image-20230209113421705.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6e9db6f214f54e0f8352dbd40f1b2c36~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
我依然为大家提供了 [免费的博客版](https://juejin.cn/post/7197977396603256890/42118200) 和 [收费的视频版](https://link.juejin.cn?target=https%3A%2F%2Fcoding.imooc.com%2Fclass%2F608.html) 。如果想要了解详细流程，那么可以自己进行选择。
...
![image-20230209121227934.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/164e708c40ec492aa148be1c98acabeb~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)
...
作者：程序员 Sunday
链接：https://juejin.cn/post/7197980894363156540
来源：稀土掘金
